...
class PluginTemplate(QObject):
	name		= 'plugin master'
	version		= '1.0'
	config		= SettingsINI(C.MITMPROXY_INI)
	loggers		= {}
	send_output = pyqtSignal(object)

	def init_logger(self,session):
		self.loggers['Pickle-Proxy'] = self.setup_logger('Pickle-Proxy',
					'logs/AccessPoint/pickle-proxy.log',session)
		self.log = self.loggers['Pickle-Proxy']
