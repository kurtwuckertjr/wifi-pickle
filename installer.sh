...
func_Banner(){
	clear
	echo '   ============================='
	echo "   |$bldblu wifi-pickle Installer$txtrst|"
	echo '   ============================='
...
func_install(){
	...
	apt-get update
	apt-get install -y python3-pip libffi-dev libssl-dev libxml2-dev libxslt1-dev zlib1g-dev
	...
	echo "[=] $bldblu Install WiFi-Pickle $txtrst"
	if [ -d "$DIRECTORY" ]; then
		rm -r $DIRECTORY
	fi
	mkdir $DIRECTORY
	cp -r $path_install /usr/share/WiFi-Pickle/
	bin_install
	echo "[$green✔$txtrst] wifi-pickle installed with success"
	echo "[$green✔$txtrst] execute $bldred sudo wifi-pickle$txtrst in terminal"
...
bin_install(){
    ...
    if which update-alternatives >/dev/null; then
        update-alternatives --install /usr/bin/wifi-pickle wifi-pickle /usr/share/WiFi-Pickle/wifi-pickle 1 > /dev/null
    else
        ln -sfT /usr/share/WiFi-Pickle/wifi-pickle /usr/bin/wifi-pickle
    fi
    cp wifi-pickle.desktop /usr/share/applications/wifi-pickle.desktop
    chmod +x /usr/share/WiFi-Pickle/wifi-pickle
}
...
uninstall(){
	...
	if [  -d "$DIRECTORY" ]; then
            if which update-alternatives >/dev/null; then
                update-alternatives --remove wifi-pickle /usr/share/WiFi-Pickle/wifi-pickle > /dev/null
            else
                rm /usr/bin/wifi-pickle
            fi
            rm /usr/share/applications/wifi-pickle.desktop
	    echo "[$red_color-$txtrst] Deleted Binary:$bldwht/usr/bin/wifi-pickle $txtrst"
            echo "[$red_color-$txtrst] Delete Path:$bldwht $DIRECTORY $txtrst"
	    rm -r $path_uninstall
	else
	    echo "[$red_color✘$txtrst] wifi-pickle is not Installed"
	fi
}
func_Banner
DIRECTORY="/usr/share/WiFi-Pickle"
Dir_isntall=$(pwd)
