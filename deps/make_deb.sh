...
DEB_ROOT="deb_tmp/wifi-pickle_${VERSION}_$ARCHITECTURE"
INSTALL_PATH=/usr/share/WiFi-Pickle
...
cp wifi-pickle.desktop ${DEB_ROOT}/usr/share/applications/wifi-pickle.desktop
...
if which update-alternatives >/dev/null; then
    update-alternatives --remove wifi-pickle $INSTALL_PATH/wifi-pickle
else
    rm -rf /usr/bin/wifi-pickle
fi
...
if which update-alternatives >/dev/null; then
    update-alternatives --install /usr/bin/wifi-pickle wifi-pickle $INSTALL_PATH/wifi-pickle 1
else
    ln -sfT $INSTALL_PATH/wifi-pickle /usr/bin/wifi-pickle
fi
chmod +x $INSTALL_PATH/wifi-pickle
chown root:root /usr/share/applications/wifi-pickle.desktop
EOF
...
cd deb_tmp
dpkg-deb --build wifi-pickle_${VERSION}_${ARCHITECTURE}
cd ..
