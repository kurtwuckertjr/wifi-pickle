...
class Refactor:
    ...
    def exportHtml(unchecked={},sessionID='',dataLogger=[],APname=''):
        ...
        contenthtml,HTML,emptyFile,activated_Files = Refactor.htmlContent('WiFi-Pickle Report Logger'),'',[],[]
        ...
        for key in readFile.keys():
            if len(readFile[key][list(readFile[key].keys())[0]]) > 0:
                HTML += '</span><span class="s2">-[ {} Logger ]-</span><span class="s1">\n'.format(key)
                HTML += readFile[key][list(readFile[key].keys())[0]]
                HTML += '</span><span class="s4"><br><br></span><span class="s1">\n'
                activated_Files.append(key)
            elif Refactor.getSize(list(readFile[key].keys())[0]) == 0:
                emptyFile.append(key)
        HTML += '</span></pre>\n<TABLE CELLSPACING=0 CELLPADDING=5 COLS=1 WIDTH="100%" BGCOLOR="#C0C0C0" >\
        <TR><TD><CENTER>''<FONT FACE="Arial, Helvetica" COLOR="#000000">WiFi-Pickle (C) 2015-2017 P0cL4bs Team' \
        '</FONT></center></TD></TR></TABLE></body>\n</html>\n'

        Load_ = {'HTML': HTML,'Files':[list(readFile[x].keys())[0] for x in readFile.keys()],
