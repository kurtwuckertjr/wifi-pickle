...
class dockPickleProxy(QTableWidget):
    ''' get all output and filter data from Pickle-Proxy plugin'''
    def __init__(self, parent=None,info={}):
        super(dockPickleProxy, self).__init__(parent)
        self.logger = info
        self.processThread = None
        self.pluginsName = []
        self.setColumnCount(2)
    ...
    def get_AllPluginName(self):
        ''' get all name plugins PickleProxy'''
        if load_plugins:
