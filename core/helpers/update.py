...
"""
Description:
    This program is a module for wifi-pickle.py. GUI update from github
...
class frm_githubUpdate(PickleModule):
    ...
    def Get_ContentUrl(self,data):
        ...
        self.connect(self.git,QtCore.SIGNAL('Activated ( PyQt_PyObject ) '), self.RcheckCommits)
        self.git.start()
        self.btnCheck.setDisabled(True)
    ...
    def RcheckCommits(self,commits):
        if 'no changes into' in commits:
            ...
            return self.btnCheck.setEnabled(True)
        elif 'New version available WiFi-Pickle v' in commits:
            ...
            "Already up-to-date. Please restart WiFi-Pickle to apply this update.")
            self.btnUpdate.setDisabled(True)
        else:
